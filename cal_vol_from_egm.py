...
def get_sample_indices_within_woi(electric, buffer=50, indices=None):
    ...
    if indices is None:
        ...
        ref_annotations = electric.annotations.reference_activation_time[indices, np.newaxis]
...
def calculate_voltage_from_electrograms(electric, buffer=50, bipolar=True, indices=None):
    """
    ...
    if bipolar:
        electrograms = electric.bipolar_egm.egm.copy() if indices is None else electric.bipolar_egm.egm[indices].copy()
    else:
        # Use only the proximal unipolar data
        electrograms = electric.unipolar_egm.egm[:, :, 0].copy() if indices is None else electric.unipolar_egm.egm[indices, :, 0].copy()

    print("### Length of electrograms: ", len(electrograms))
    sample_within_woi = get_sample_indices_within_woi(electric, buffer=buffer, indices=indices)
...
if __name__ == "__main__":
    ...
    interpolator_cal_voltage = interpolators.LocalSMoothingInterpolator(
    points=electric.bipolar_egm.points,
    field=cal_voltages)
    ...
    plotter = draw_map(
        mesh=mesh,
        field=interpolated_cal_voltage,
        field_type="voltage"
    )
    plotter.show()
