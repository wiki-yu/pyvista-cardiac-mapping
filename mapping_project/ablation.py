...
def extract_ablation_data(ablation_data):
    ...
    if (
        isinstance(ablation_data, np.ndarray)
        or ablation_data["originaldata"]["ablparams"]["time"].size == 0
    ):
    ...
    force = AblationForce(
        ...
        lateral_angle=ablation_data["originaldata"]["force"]["lateralangle"].astype(
            float
        ),
