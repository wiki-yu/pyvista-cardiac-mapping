...
def get_mapping_points_within_woi(case, indices=None, buffer=50):
    ...
    local_activation_times = (
        local_activation_times[indices]
        if indices is not None
        else local_activation_times
    )
...
def get_electrograms_at_points(
    ...
    if case.electric.internal_names is not None:
        ...
        names = np.asarray(
            [f"P{index}" for index in range(len(electrograms))], dtype=str
        )
    ...
    if indices is not None:
        ...
        # if we have a single index we need to ensure it is an array
        indices = (
            np.asarray([indices], dtype=int) if isinstance(indices, int) else indices
        )
    ...
    if within_woi:
        points_within_woi = get_mapping_points_within_woi(
            case, indices=indices, buffer=buffer
        )
...
def get_sample_indices_within_woi(case, buffer=50, indices=None):
    ...
    if indices is None:
        ...
        ref_annotations = case.electric.annotations.reference_activation_time[
            :, np.newaxis
        ]
    else:
...
def calculate_voltage_from_electrograms(case, buffer=50, bipolar=True, indices=None):
    ...
    if bipolar:
        electrograms = (
            case.electric.bipolar_egm.egm.copy()
            if indices is None
            else case.electric.bipolar_egm.egm[indices].copy()
        )
    else:
        # Use only the proximal unipolar data
        electrograms = (
            case.electric.unipolar_egm.egm[:, :, 0].copy()
            if indices is None
            else case.electric.unipolar_egm.egm[indices, :, 0].copy()
        )

    sample_within_woi = get_sample_indices_within_woi(
        case, buffer=buffer, indices=indices
    )
...
def calculate_points_within_distance(
    origin, destination, max_distance, return_distances=True
):
    """
...
def interpolate_voltage_onto_surface(
    ...
    if bipolar:
        ...
        points = case.electric.unipolar_egm.points[
            include, :, 0
        ]  # Use only the proximal unipolar data
