...
class Annotations:
    ...
    def __repr__(self):
        n_points = (
            len(self.window_of_interest) if self.window_of_interest is not None else 0
        )
...
def extract_electric_data(electric_data):
    ...
    if "electrodeNames_bip" not in electric_data:
        electric_data["electrodeNames_bip"] = np.full_like(
            internal_names, fill_value="", dtype=str
        )
    if "egmUni" not in electric_data:
        num_points, num_samples = electric_data["egm"].shape
        electric_data["egmUni"] = np.full(
            (num_points, num_samples, 2), fill_value=0.0, dtype=float
        )
        electric_data["egmUniX"] = np.full(
            (num_points, 3, 2), fill_value=np.NaN, dtype=float
        )
        electric_data["voltages"]["unipolar"] = np.full(
            num_points, fill_value=np.NaN, dtype=float
        )
        electric_data["electrodeNames_uni"] = np.full_like(
            internal_names, fill_value="", dtype=str
        )

    # Not all datasets have gain values
    if "egmGain" not in electric_data:
        electric_data["egmGain"] = np.full(
            electric_data["egmRef"].shape[0], fill_value=1.0, dtype=float
        )
    if "egmUniGain" not in electric_data:
        electric_data["egmUniGain"] = np.full(
            electric_data["egmRef"].shape[0], fill_value=1.0, dtype=float
        )
    if "egmRefGain" not in electric_data:
        electric_data["egmRefGain"] = np.full(
            electric_data["egmRef"].shape[0], fill_value=-4.0, dtype=float
        )
    if "ecgGain" not in electric_data:
        electric_data["ecgGain"] = np.full(
            electric_data["egmRef"].shape[0], fill_value=1.0, dtype=float
        )
    ...
    annotations = Annotations(
        ...
        reference_activation_time=electric_data["annotations"]["referenceAnnot"].astype(
            float
        ),
    )
